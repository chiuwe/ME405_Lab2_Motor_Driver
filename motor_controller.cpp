...
void motor_controller::run (void) {
   ...
   // outputs 16E6/64/255 = 980Hz PWM
	motor_driver *p_my_motor_driver1 = new motor_driver(p_serial, &DDRC, 0x07, &DDRB, 0x40, &PORTC, 0x04, &TCCR1A, 0xA9, &TCCR1B, 0x0B, &OCR1B);
	motor_driver *p_my_motor_driver2 = new motor_driver(p_serial, &DDRD, 0xE0, &DDRB, 0x20, &PORTD, 0x80, &TCCR1A, 0xA9, &TCCR1B, 0x0B, &OCR1A);
   ...
   PORTC |= (1 << 3) | (1 << 4);
	...
	for (;;) {
      if (PINC & (1 << 3)) {
         ...
         DBG(p_serial, "motor 1 breaking" << endl);
      } else {
      ...
      if (PINC & (1 << 4)) {
         ...
         DBG(p_serial, "motor 2 breaking" << endl);
      } else {
